...
- Evidence freshness: historical entries retain their original verification dates; check the real checkout and current build before relying on them.
- Safe continuation: `yes` — check `git status -sb` for the current branch and unpushed commits. Anything that restarts `coreaudiod` or publishes needs the user's approval (see Safety Boundaries).
...
- `[VERIFIED 2026-09-26]` The independent SSD reader accepts Version 0.1 and 0.3, validates the five-column 0.3 object-linked Dante map, and rejects non-identity channel maps because meters and Test Signal currently use one channel number. Synthetic parser checks passed, all four current private Room A/B/C/FIL layouts parsed, and the bundled app's Room C docshot showed 24 speakers on 24 channels with no load error. A/B/C have 16/17/24 playable channels; FIL has no `[SPEAKER]` assignment. The local Layouts copies were updated from the current canonical files and byte-compared. `[NOT_RUN]` No physical or Dante output was tested for this change.
- `[VERIFIED 2026-09-25]` The test-signal status now caps the visible speaker label at eight characters (with an ellipsis), shows the complete list in a tooltip, and always reserves a 120 pt status slot so playback does not move the controls. A synthetic long-name `--docshot ... --long-status-preview` and an idle docshot at 1200 pt were visually compared; the controls stayed aligned and visible. Swift debug build passed. The running app was not restarted for this UI change.
- `[VERIFIED 2026-09-25]` Test signal has a separate 4 Hz, 50% duty pink-noise pulse choice. Each channel/signal switch resets its pulse phase; the 125 ms ON window has 2 ms edge ramps and the 125 ms OFF window is silent. SSD-speaker/all-channel stepping has a 0.5 s minimum dwell. `make -C Tools check` measured two ON and two silent OFF windows in 0.5 s; `swift build --package-path VisualizerApp` passed. `[NOT_RUN]` The currently running app was not restarted, and physical audio output was not exercised for this change.
- `[VERIFIED 2026-09-25]` A synthetic `--docshot ... --test-preview` rendered a blue TEST preview in Meters, the matching SSD speaker in Monitor, and speaker-table target values. A Room C docshot also showed an unassigned TEST channel in Meters → Layout with no `[SPEAKER]` mapping. The implementation sources this preview from the test engine's selected channel and requested dBFS without sending audio into the virtual device; measured virtual levels and live routing warnings remain separate. `[NOT_RUN]` External playback with this new UI path was not exercised on Dante or physical hardware, and receiver audio is not verified.
- `[VERIFIED 2026-09-25]` Test signal now selects exactly one Core Audio output by stable device UID. The GUI defaults to the virtual device, also lists Dante Virtual Soundcard and other available outputs, and remembers the selected UID. Switching devices, disappearance, format changes, or a Core Audio restart stop playback without automatic fallback or restart. An external selection starts at no more than −40 dBFS; all-channels-at-once is available only for the virtual device. There is no audio input or pass-through path. `--list-test-outputs` enumerates outputs without playing; `--test-signal` continues to use the virtual device regardless of the saved GUI selection.
- `[VERIFIED 2026-09-25]` Swift build, `make -C Tools check`, universal `build_app.sh`, GUI docshot, and a brief −60 dBFS virtual-device CLI signal passed. The live output inventory included Virtual Audio Interface (128ch) and Dante Virtual Soundcard (64ch). `[NOT_RUN]` No test tone was sent to Dante Virtual Soundcard or a physical interface; receiving hardware, Dante subscriptions, and acoustic output are not verified.
...
- `[HISTORICAL 2026-09-24]` Before the 0.3 map and the current Room A/C speaker assignments, the local layouts had FIL-v1 35 objects/0 SPEAKER rows, Room A 2/0, Room B 61/24, Room C 76/0. This snapshot is superseded by the 2026-09-26 verification above.
