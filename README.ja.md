...
外部出力中はMetersに青い**TESTプレビュー**として指定chと設定レベルを表示し、Monitorの対応するSSDスピーカーと
スピーカー一覧も青く反応します。これは送信予定の表示であり、出力先での受信・実測値ではありません。仮想デバイスの
実測メーターとルーティング警告は分けて表示します。`[SPEAKER]`割り当てがないレイアウトでもMetersのLayout表示に
未割当のTEST chを表示します。
