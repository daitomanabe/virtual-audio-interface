...
- **Apply SSD gain**(既定 ON)では、入力レベル + `Gain`(スピーカーから出る想定のレベル)でスピーカーを
  光らせます。スピーカー一覧には両方の値を表示し、レベルバーは 3D 表示の基準になっている列に出ます。
  Mute のスピーカーは信号があっても光りません(赤い ×)。**Ch + Name** ラベルには 0 でない `Gain` / `Delay` を
  添えます。

### シーンの要素

**Scene objects**(既定 ON)のとき、スピーカー以外の OBJECT を控えめな色で描きます。
...
```bash
make -C Tools check            # 共有メモリとメーターの減衰計算、SSD パーサー、シーン要素と座標変換、ファイル監視
...
- **UI の洗練**: デザインの統一(文字、余白、色、ライト / ダーク)、アプリアイコン、メーターのレイアウト切替と
  スピーカー層ごとのグループ表示、近いスピーカーのラベルの重なり解消、Delay の図示、
